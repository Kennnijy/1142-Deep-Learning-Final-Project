...
print("  ok     (OK 手勢)           → 左鍵點擊")
print("  switch (讚 手勢)           → 切換桌面 (Ctrl+Win+left/right)\n")
...
while cap.isOpened():
    ...
    if result.hand_landmarks:
        ...
        if gesture == "one":
            ...
            # 1. 正常的正規化計算（去掉前面的負號）
            raw_x = max(0.0, min(1.0, (lms[8].x - MOUSE_MARGIN) / (1 - 2 * MOUSE_MARGIN)))
            norm_y = max(0.0, min(1.0, (lms[8].y - MOUSE_MARGIN) / (1 - 2 * MOUSE_MARGIN)))
            
            # 2. 因為攝影機有做 cv2.flip(frame, 1) 鏡像，手勢的 X 座標需要反轉，滑鼠方向才會跟手同步
            norm_x = 1.0 - raw_x 
